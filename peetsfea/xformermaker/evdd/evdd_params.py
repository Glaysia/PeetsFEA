import numpy as np
from peetsfea.util import abstract_parameter
import math


class Parameter(abstract_parameter):
    ...
    def get_random_variable(self):
        ...
        #######################################
        self.Tx_width = round(
            math.sqrt(self.wire_diameter**2*self.strand_number*2), 2)
        self.Tx_height = self.Tx_width

        Tx_max = (self.Tx_layer_space_y + self.Tx_width) * \
            math.ceil(self.Tx_turns) + self.Tx_space_y - self.Tx_layer_space_y
        ...
        while (True):
            if self.Tx_height*2 + self.Tx_preg + self.Rx_height*2 + self.Rx_preg * 2 >= self.h1:
                ...
                self.Tx_layer_space_y = self._random_choice(
                    Tx_layer_space_y_range)
                # self.Tx_width = self._random_choice(Tx_width_range)
                self.wire_diameter = self._random_choice(wire_diameter_range)
                self.strand_number = self._random_choice(strand_number_range)
                self.Tx_width = round(
                ...
                Tx_max = (self.Tx_layer_space_y + self.Tx_width) * \
                    math.ceil(self.Tx_turns) + self.Tx_space_y - \
