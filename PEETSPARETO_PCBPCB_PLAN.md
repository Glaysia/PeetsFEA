...
- Align transformer defaults with whatever reference dataset/product brief the Product Lead finalizes, and flag any schema drift back to them.
...
- Implement config merge logic (defaults + user overrides) and provide fixtures for QA.
...
2. **Schema Finalization** — Agent A delivers typed specs + validators, unblocking Agents B/C.
