...
class Sim(Parameter) :
    ...
    def create_winding(self) :
        ...
        for i in range(0,math.ceil(self.Tx_turns)) :
            self.temp_Tx.append([f"-(w1*w1_ratio/2 + Tx_space_x + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_x + Tx_width))" ,f"-(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-{i})*(Tx_layer_space_y + Tx_width))" ,f"0"])
            self.temp_Tx.append([f"-(w1*w1_ratio/2 + Tx_space_x + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_x + Tx_width))" ,f"(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-{i})*(Tx_layer_space_y + Tx_width))" ,f"0"])
            self.temp_Tx.append([f"(w1*w1_ratio/2 + Tx_space_x + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_x + Tx_width))" ,f"(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-{i})*(Tx_layer_space_y + Tx_width))" ,f"0"])
            if i == math.ceil(self.Tx_turns) - 1 :
                self.temp_Tx.append([f"(w1*w1_ratio/2 + Tx_space_x + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_x + Tx_width))" ,f"(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_y + Tx_width))" ,f"(Rx_preg + Rx_height + Tx_preg + Tx_height)"])
                self.temp_Tx.append([f"(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)" ,f"(l1_center/2 + Tx_space_y + Tx_width/2)" ,f"(Rx_preg + Rx_height + Tx_preg + Tx_height)"])
            else :
                self.temp_Tx.append([f"(w1*w1_ratio/2 + Tx_space_x + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}))*(Tx_layer_space_x + Tx_width))" ,f"-(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1-({i}+1))*(Tx_layer_space_y + Tx_width))" ,f"0"])

        self.Tx_1 = self._create_polyline_litz(points = self.temp_Tx, name = f"Tx_1", coil_width = "Tx_width")
    ...
    def create_exctation(self) :
        self.Tx_in = self.M3D.modeler.get_faceid_from_position(position=[f"(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)" ,f"-(l1_center/2 + Tx_space_y + Tx_width/2 + ({math.ceil(self.Tx_turns)}-1)*(Tx_layer_space_y + Tx_width))" ,"(Rx_preg + Rx_height + Tx_preg + Tx_height)"])
        self.Tx_out = self.M3D.modeler.get_faceid_from_position(position=[f"(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)" ,f"(l1_center/2 + Tx_space_y + Tx_width/2)" ,f"(Rx_preg + Rx_height + Tx_preg + Tx_height)"])
                                                                        
        self.Rx2_in = self.M3D.modeler.get_faceid_from_position(position=[f"-(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)",f"-(l1_center/2 + Rx_space_y + Rx_width/2 + Tx_space_y + Tx_width + ({self.Tx_turns}-1)*(Tx_layer_space_y + Tx_width))","-(Rx_preg/2+Rx_height/2)"])
        self.Rx2_out = self.M3D.modeler.get_faceid_from_position(position=[f"-(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)",f"(l1_center/2 + Rx_space_y + Rx_width/2 + Tx_space_y + Tx_width + ({self.Tx_turns}-1)*(Tx_layer_space_y + Tx_width))","-(Rx_preg/2+Rx_height/2)"])

        self.Rx1_in = self.M3D.modeler.get_faceid_from_position(position=[f"-(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)",f"-(l1_center/2 + Rx_space_y + Rx_width/2 + Tx_space_y + Tx_width + ({self.Tx_turns}-1)*(Tx_layer_space_y + Tx_width))","(Rx_preg/2+Rx_height/2)"])
        self.Rx1_out = self.M3D.modeler.get_faceid_from_position(position=[f"-(w1*w1_ratio/2 + ({self.terminal}mm) + 40mm)",f"(l1_center/2 + Rx_space_y + Rx_width/2 + Tx_space_y + Tx_width + ({self.Tx_turns}-1)*(Tx_layer_space_y + Tx_width))","(Rx_preg/2+Rx_height/2)"])
    ...
    def _get_copper_loss_parameter(self) :
        ...
        report = self.M3D.post.create_report(expressions=result_expressions, setup_sweep_name=None, domain='Sweep', 
                                        ...
                                        report_category=None, plot_type='Data Table', context=None, subdesign_id=None, polyline_points=1001, plotname="copper loss data")
        dir_data = self.M3D.post.export_report_to_csv(project_dir=self.dir, plot_name=report.plot_name)
    ...
    def get_input_parameter(self) :
        ...
        input_parameter_columns = ["freq","input_voltage","w1","l1_leg","l1_top","l2","h1","l1_center",
    ...
    def coreloss_project(self) :
        ...
        # 코어손실 계산할 자화 전류 인가
        self.magnetizing_current = self.input_voltage*math.sqrt(2)/2/math.pi/(self.freq*10**(3))/self.Lmt/10**(-6) #V*sqrt(2)/wL
        ...
        report = self.M3D.post.create_report(expressions=data5_result, setup_sweep_name=None, domain='Sweep', 
                                        variations={"magnetizing_current": ["All"]}, primary_sweep_variable=None, secondary_sweep_variable=None, 
                                        report_category=None, plot_type='Data Table', context=None, subdesign_id=None, polyline_points=1001, plotname="magnetizing copper loss data")
        dir_data = self.M3D.post.export_report_to_csv(project_dir=self.dir, plot_name=report.plot_name)
...
for i in range(5000):
    ...
    try:
        # 시뮬레이션 코드 실행
        sim.simulation()
        pid = sim.desktop.aedt_process_id
        sim._close_project()
        try:
            sim.desktop.release_desktop()
        except AttributeError:
            sim.desktop.close()
        project_dir = os.path.join(sim.dir_temp,'script',f'script{sim.num}')
        time.sleep(1)
        ...
        try:
            ...
            print(f"[DEBUG] Deleted {project_dir}")
        except Exception as e:
            print(f"[ERROR] Could not delete {project_dir}: {e}")

        log_simulation(number=sim.num, state="finished")
        loging(f"{sim.PROJECT_NAME} : simulation success!! {i} ({execution_time:.1f} sec)")
        ...
        loging(f"{sim.PROJECT_NAME} : {i} simulation Failed")
        sim._close_project()
        try:
            sim.desktop.release_desktop()
        except AttributeError:
